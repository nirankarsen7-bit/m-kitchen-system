...
export const DashboardLive: React.FC = () => {
  ...
  // Count active occupied tables (status = active) and list their numbers (F8)
  const occupiedTablesList = tables.filter(t => t.status === TableStatus.ACTIVE);
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* F8: Show occupied table numbers */}
        <Card className="bg-white border-gold-rich/10 relative overflow-hidden p-5">
          ...
          <div className="text-[10px] text-mocha mt-2 font-medium">
            {occupiedTables > 0 ? (
              <span className="flex flex-wrap gap-1 mt-1">
                {occupiedTableNumbers.slice(0, 10).map(num => (
                  <span key={num} className="px-1.5 py-0.5 bg-maroon-royal/10 text-maroon-royal rounded text-[9px] font-bold">T{num}</span>
                ))}
                {occupiedTableNumbers.length > 10 && (
                  <span className="text-[9px] text-mocha">+{occupiedTableNumbers.length - 10} more</span>
                )}
              </span>
            ) : (
              <span>No occupied tables</span>
            )}
          </div>
        </Card>

        {/* F2: Emergency highlight for pending approvals */}
        <Card className={`relative overflow-hidden p-5 ${pendingApprovalsList.length > 0 ? "bg-red-50 border-2 border-warning animate-pulse ring-4 ring-warning/20" : "bg-white border-gold-rich/15"}`}>
          <div className="absolute right-3 top-3 text-warning/50"><BellRing className={`w-12 h-12 ${pendingApprovalsList.length > 0 ? "animate-bounce" : ""}`} /></div>
          ...
          <div className="text-[10px] text-mocha mt-2 font-medium flex items-center gap-1">
            {pendingApprovalsList.length > 0 ? (
              <>
                <AlertTriangle className="w-3.5 h-3.5 text-warning animate-bounce" />
                <span className="text-warning font-bold">URGENT: Requires action!</span>
              </>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Right Column (40%): Supplemental approvals feed - F2 emergency highlight */}
        <div className="lg:col-span-5 space-y-4">
          <div className={`flex items-center justify-between pb-2 border-b ${pendingApprovalsList.length > 0 ? "border-warning/30" : "border-gold-rich/15"}`}>
            <h3 className="font-serif text-lg font-bold text-maroon-royal flex items-center gap-2">
              <BellRing className={`w-4.5 h-4.5 ${pendingApprovalsList.length > 0 ? "text-warning animate-bounce" : "text-gold-rich"}`} />
              Pending Approvals
            </h3>
            <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold font-mono tracking-wider ${pendingApprovalsList.length > 0 ? "bg-warning text-white animate-pulse" : "bg-gold-rich/20 text-gold-rich"}`}>
              {pendingApprovalsList.length > 0 ? "URGENT ACTION" : "Requires validation"}
            </span>
          </div>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              {pendingApprovalsList.map(item => {
                ...
                return (
                  <div
                    key={item.id}
                    className="p-4 bg-red-50 border-2 border-warning rounded-2xl relative shadow-lg animate-pulse border-warning overflow-hidden"
                  >
                    {/* Emergency indicator */}
                    <div className="absolute top-0 left-0 right-0 h-1 bg-warning animate-pulse" />

                    <div className="flex justify-between items-start pb-2.5 mb-2.5 border-b border-warning/20">
                      <div>
                      ...
                      <span className="px-2 py-0.5 rounded bg-warning text-white text-[9px] uppercase font-bold tracking-wider animate-bounce flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> Wants Addition
                      </span>
                    </div>
      ...
      {/* F10: Remove quick actions from reception (only show for admin) */}
      {isAdmin && (
