...
// Raw material per-plate usage tracking (F11)
export interface MaterialUsage {
  id: string;
  menu_item_id: string;
  material_name: string;
  quantity_per_plate: number;
  unit: string; // kg, g, litres, ml, etc.
}

// Supplier payment with proof (F16)
export interface SupplierPayment {
  ...
  notes?: string;
  proof_image_url?: string; // base64 or URL
  created_at: string;
}

// Coupon auto-generation settings (F6)
export interface CouponSettings {
  min_purchase_for_coupon: number;
  coupon_discount_percent: number;
  coupon_validity_days: number;
  coupon_min_purchase_next: number;
}
